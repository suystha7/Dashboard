  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        if (!response.ok)
          throw new Error(
            `Failed to ${isEditMode ? "update" : "create"} activities`
          );

        const data = await response.json();
        onSubmit(data, isEditMode);
        toast.success(
          `Activities ${isEditMode ? "updated" : "created"} successfully!`
        );
  ...
  return (
    <div>
      <Dialog>
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? "Edit Activities" : "Create Activities"}
          </DialogTitle>
        </DialogHeader>
      </Dialog>
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-4 mt-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="image" name="image" {...formik.getFieldProps("image")} />
