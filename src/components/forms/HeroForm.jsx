...
export default function HeroForm({ onSubmit, onCancel, initialData = null }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        if (!response.ok)
          throw new Error(`Failed to ${isEditMode ? "update" : "create"} hero`);

        const data = await response.json();
        onSubmit(data, isEditMode);
        toast.success(
          `Hero ${isEditMode ? "updated" : "created"} successfully!`
        );
  ...
  return (
    <div>
      <Dialog>
        <DialogHeader>
          <DialogTitle>{isEditMode ? "Edit Hero" : "Create Hero"}</DialogTitle>
        </DialogHeader>
      </Dialog>
